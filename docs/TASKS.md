...
- **Phase 2コア: `PurchaseAgreement`（買取契約）実装**（2026-08-16。[ADR-0087](DECISIONS.md)。旧`ConsignmentAgreement`から改称・改修）
  ...
  - **検証済み**（2026-08-16、C:\psa）: `tsc --noEmit`エラー0件、`npm run build`成功（57ルート、Prisma Client再生成込み）。ビルド中の`prisma.legalDocument.findUnique()`エラーログはローカルDB未接続時の静的生成試行によるもので、ビルド自体は失敗していない
...
**未着手（実装）**
- Phase 1残・Phase 2残とも: `db push`でのスキーマ反映確認、実データでの動作確認（型チェック/ビルドは2026-08-16確認済み、上記参照）
