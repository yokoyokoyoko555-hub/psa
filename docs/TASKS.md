...
> 凡例: ✅ 実装済 / 🟡 部分実装・要改善 / ❌ 未実装
> 最終更新: 2026-06-20。タスク着手・完了時はこの表を更新すること。
...
- 🟡 **代理申込の決済通電** — 画面・データ・フローは実装済み（顧客の依頼→管理「要対応」→店舗入力→確定）。通常申込の Stripe Elements は実装済み。残りは「代理申込依頼時のカード登録」と「確定時の登録カードへ off_session 即時決済」（[ADR-0011](DECISIONS.md)）
- ❌ **PSA US の正式料金** — 現在JPと同額の暫定値。管理画面→設定で正式値に更新
- 🟡 **Stripe Elements 統合** — 通常申込の実カード決済は実装済み。代理申込のカード登録/即時決済への横展開は未実装
...
- 🟡 **Stripe/S3/SMTP の本番接続**（env未設定。決済・画像アップロード・メールを使う前に設定。特にSMTP設定は本番メール送信前に必須）

### 優先度: 中
- 🟡 申込フォームの一時保存はlocalStorage実装（同一端末のみ復元可）。複数端末で再開するならDBドラフト化が必要
- ❌ 顧客側のパスワード変更（管理側は実装済み。Customer向けは未実装）
- ❌ 顧客プロフィール編集（住所変更等）
- 🟡 お知らせ/通知（全体向けの画面表示・作成・一時保存・公開/非公開・表示対象選択・編集・削除は実装済み。個別配信/既読管理は未実装）
- ❌ 返却配送の追跡番号入力・顧客通知
