...
- 日付: 2026-08-11
- 状態: Accepted（事業スキーム転換・設計方針確定。詳細スキーマ・画面は未実装）。[ADR-0087](DECISIONS.md)により事業スキームは条件付買取へ再転換されSuperseded（委託販売（問屋型）への転換自体は撤回）。
...
## ADR-0086: 「未確定の事業判断」と「Phase 2実装のブロッカー」を混同していたのを訂正し、ConsignmentAgreement実装に着手

- 日付: 2026-08-11
- 状態: Accepted
- 文脈: これまでのADRで「要事業判断」として残していた項目のうち、実際にはコード実装そのものをブロックしないものを、横山から指摘を受けて再整理した。
- 決定:
  1. **手数料率テーブルの実際の%は`ConsignmentAgreement`実装のブロッカーではない**。`/admin/ebay/settings`は元々空でも動く設計（[ADR-0078](DECISIONS.md)のCommissionRateTierと同じ）。数値未入力のまま実装を進める。
  2. **輸出配送業者はDHLで確定**。署名基準等の細部はPhase 5（発送作業）着手時に確定すればよく、Phase 2の実装とは無関係だった。
  3. **委託・問屋営業の法令要件確認（本人確認要否）は回答を待たずに実装を進める**。[ADR-0078](DECISIONS.md)で決めた通り、身分証画像アップロード機能は土台として用意しオン/オフを切り替えられる設計にする（デフォルトはオフ、確認結果が来たら必要に応じて有効化）。
  4. **返品時のリスク配分の詳細ルールもブロッカーではない**。「精算後の返品は差額請求する」という大枠（[ADR-0085](DECISIONS.md)決定5）さえあれば実装でき、返送送料・為替差損・eBay手数料の具体的な配分は固定ロジックにせず、ADMINが金額を都度入力できる形にしておけば後から運用ルールを調整できる。
  5. 上記を踏まえ、`ConsignmentAgreement`（委託契約・電子同意・出品条件提示。Phase 2）の実装に着手する。
- 影響: `ConsignmentAgreement`実装のスコープには影響しない（[ADR-0078](DECISIONS.md)/[ADR-0085](DECISIONS.md)の設計をそのまま使う）。教訓として、以後「要事業判断」を記録する際は「実装をブロックするか」「後から設定・調整可能な運用値に過ぎないか」を区別して記載する。
- 未対応: なし（本ADRは方針転換の記録のみ）。

## ADR-0087: 事業スキームを委託販売（問屋型）から条件付買取へ再転換（ADR-0078の再改訂）

- 日付: 2026-08-11
- 状態: Accepted（設計確定。スキーマ・Server Action・顧客/管理画面UI実装済み。eBay出品API連携等のPhase 3以降は未着手）
- 文脈: 消費税の輸出免税・問屋取引の扱いを調査する中で、横山より「オークション形式に限定した場合、売値を顧客・当社どちらも決めない（市場が決める）ため、経済的には『売れたら買取』と同じ結果になるのでは」という指摘があった。[ADR-0078](DECISIONS.md)で買取から委託販売（問屋型）へ転換した理由は2つ（①古物営業法の非対面本人確認義務、②固定買取価格の査定・改定・買取代金管理の運用負荷）だったが、**オークション形式を前提にすれば、買取価格を「落札額－手数料」として事後的に算出できるため②は解消する**（元の仕様書v0.1「条件付買取」の考え方そのもの）。①（本人確認義務）は買取に戻すと原則として必要になる（委託販売でグレーだったのは所有権を当社が取得しないことが理由だったため）が、横山は**本人確認をアカウント単位で出品申請前に事前完了させる運用にする**ことでこれを許容し、買取へ戻すことを決定した。
- 決定:
  1. **事業スキームを条件付買取（旧仕様書v0.1の考え方）へ戻す**。当社が顧客からカードを買い取り、当社所有として海外へ輸出販売する。買取価格は事前の固定合意ではなく、**eBayオークションで落札が成立した後に「落札額－手数料」から算出**する（条件成就型）。
  2. **本人確認はアカウント単位・出品申請前の事前完了制**とする。カードごとの申請時に都度確認するのではなく、`Customer.identityVerifiedAt`が設定済みであることを、委託（買取）申請フローの入口条件にする。確認フロー（身分証画像アップロード＋スタッフ審査）を実際に実装する（[ADR-0086](DECISIONS.md)で「土台のみ・オフ」としていたトグルを廃止し、必須の実機能にする）。
  3. **所有権モデルを4状態に戻す**: `CUSTOMER_OWNED`→`PURCHASE_RESERVED`（買取条件成就待ち）→`COMPANY_OWNED`（買取成立・当社所有）→`SOLD_TO_BUYER`。委託版の`CONSIGNED`状態は廃止する。
  4. **`ConsignmentAgreement`は`PurchaseAgreement`（買取契約）へ改称・改修する**。ゼロから作り直すのではなく、既存の申請→審査→電子同意のフロー構造（`requestConsignment`→`reviewConsignmentAgreement`→`agreeConsignmentAgreement`に相当する3ステップ）を流用し、買取条件成就（eBay落札）時の所有権移転・買取代金債務作成のロジックを追加する。落札前は`startingPriceUsdMinor`/`reservePriceUsdMinor`（オークション条件）を持ち、落札後に`purchasePriceJpy`（買取価格、円）を算出する2段階の価格情報を持つ。
  5. 精算モデル（旧`ConsignmentSettlement`/`SettlementPayment`）も、旧仕様書v0.1の`CardPurchase`/`PurchasePayment`（買取・買取代金支払い）の考え方に戻す。`antiqueLedgerReference`（古物台帳参照）フィールドを追加し、古物営業法上の記帳義務との接続を用意する。
  6. eBay側の技術設計（オークション形式・7日サイクル・委託契約有効期限との自動再出品連動・Media API画像・プラットフォーム非依存設計）は変更しない。オークション自体の仕組みは買取/委託どちらでも同じであるため。
- 影響: `docs/EBAY_CONSIGNMENT_SALES_SPEC.md`は再度全面改訂が必要（ファイル名は当面维持、内容は買取前提に戻す）。`prisma/schema.prisma`の`ConsignmentAgreement`/`CardOwnershipStatus`/関連actionsを改修する。本人確認の実装（身分証アップロード・審査画面）が新規タスクとして追加される。[ADR-0078](DECISIONS.md)〜[ADR-0086](DECISIONS.md)のうち委託販売固有の内容（問屋型の法的構造、Best Offer→予約価格の置き換えなど）は本ADRにより一部Superseded。オークション・プラットフォーム非依存・手数料率テーブル・委託契約有効期限（→買取条件有効期限に読み替え）等の設計はそのまま有効。
- 未対応: 本人確認の具体的な提出書類・審査基準（[ADR-0085](DECISIONS.md)で送付済みの行政書士・警察への質問への回答を踏まえて確定）。買取価格の算出式の細部（手数料以外の控除項目の有無）。古物台帳との具体的な連携方法。
