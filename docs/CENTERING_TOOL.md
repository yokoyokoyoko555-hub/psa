...
`@@map("centering_measurements")`、`@@index([customerId])`。撮影画像は保存しない（方針通り）。`detectionSample`(Json) のみ座標テレメトリを保持（下記）。

---

## 4.5 入力経路と学習ロードマップ（ADR-0013）

### 入力経路
- **撮影**（カメラ）／**画像取り込み**（保存済み画像のアップロード）の両方に対応。どちらも `aiEnabled` なら取り込み後にAIが自動検出→手動微調整。

### 学習による精度向上ロードマップ
AIの手動補正を活かして検出精度を継続改善する。段階:
- **① 補正テレメトリ（実装済・画像なし）**: `CenteringMeasurement.detectionSample` に「AI提案の四隅」と「ユーザー確定の四隅」を表裏それぞれ**正規化座標のみ**で記録（画像は保存しない）。AIの検出誤差・系統的ズレを分析し、CVパラメータ（Canny感度・同心判定距離・面積レンジ）調整に使う。形: `{ front: { proposed:{outer,inner}|null, final:{outer,inner} }|null, back: 同 }`。
- ② オプトインで画像＋補正座標を収集しデータセット化（プライバシーポリシー改定が前提）。
- ③ MLモデルを学習→TF.js/ONNXで端末内推論（推論コストは引き続きゼロ）。

> ①の集計・可視化（管理画面 or DBクエリ）は別途タスク。まずは収集のみ。
